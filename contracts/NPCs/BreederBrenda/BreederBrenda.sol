...
BreederBrendaStorage
{
  ...
  address public evoToken;

  /// @notice evo ERC721 nft bred
  address public evoNft;
  ...
  function requestBreed(
    ...
    // Transfer EVO tokens
    IERC20(evoToken).transferFrom(msg.sender, treasury, amount);

    emit BreedRequested(msg.sender, parent1, parent2, amount);
  }
  ...
  function mintApprovedBreed(
    ...
    IEvo(evoNft).mintTo(to, tokenId);
    emit BreedMinted(to, tokenId, msg.sender);
  }
