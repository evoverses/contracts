...
contract EvoToken is ERC20, Ownable, Authorizable {
    ...
    // We had to change it from onlyOwner to onlyAuthorized since later on the Play to Earn
    // contract will also need to mint
    function mint(address _to, uint256 _amount) public onlyAuthorized {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }

    function updateManualMintLimit(uint256 _newLimit) public onlyAuthorized {
        manualMintLimit = _newLimit;
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "EvoToken::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "EvoToken::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "EvoToken::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "EvoToken::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "EvoToken::_writeCheckpoint: block number exceeds 32 bits"
        );
