...
// MasterInvestor is the master gardener of whatever gardens are available.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once EVO is sufficiently
// distributed and the community can show to govern itself.
//
contract MasterInvestor is Ownable, Authorizable, ReentrancyGuard {
    ...
    modifier nonDuplicated(IERC20 _lpToken) {
        require(
            ...
            "MasterInvestor::nonDuplicated: duplicated"
        );
        _;
    }
    ...
    function add(
        ...
        require(
            ...
            "MasterInvestor::add: lp is already in pool"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (GovTokenForFounders > 0) {
            ...
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                govToken.lock(
                    address(founderaddr),
                    GovTokenForFounders.mul(95).div(100)
                );
            }
        }
    }
        ...
        )
    {
        ...
        uint256 amount = multiplier.mul(REWARD_PER_BLOCK).mul(_allocPoint).div(
            totalAllocPoint
        );
    ...
    function deposit(
        ...
        require(
            _amount > 0,
            "MasterInvestor::deposit: amount must be greater than 0"
        );
    ...
    function safeGovTokenTransfer(address _to, uint256 _amount) internal {
        ...
        require(
            transferSuccess,
            "MasterInvestor::safeGovTokenTransfer: transfer failed"
        );
    }
